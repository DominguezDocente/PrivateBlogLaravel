    ...
    <section class="section">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('blogs.update') }}" class="row g-3" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="form-floating">
                            <select class="form-control" name="section_id">
                                @foreach ($sections as $section)
                                    <option value="{{ $section->id }}" {{ $section->id == $blog->section_id  ? 'selected' : '' }}>
                                        {{ $section->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea id="description" name="description" class="form-control" rows="10"> {{ $blog->description }} </textarea>
                    </div>
